...
## Recursive Bayesian learning models (or simplify considerably without trial-by-trial analysis)
...
The models we considered were different implementations of the same recursive Bayesian learning model [@foley_sure_2017]. We first fitted the models to two baseline conditions in experiment 1 (BPV2 and BVP2), in order to select values for the free parameters. We then used these values for the remaining conditions in all experiments.
The models that we implemented differed in the memory of events that were tracked and the decision mechanism. These models were:

1. Hurdle model  
2. Random dimension model  
3. Winner-takes-it-all model  
4. Probability first model  
5. Volume first model  
6. Volume only model  
7. Logproduct rule model  
...
cv_fit <- function(phys1, phys2, cv, lapse) {
  (1 - lapse) * pnorm(0, mean = phys1 - phys2, sd = cv*(phys1 + phys2), lower.tail = FALSE) + lapse * 0.5
}

se <- function(x) sqrt(var(x)/length(x)) # function for calculating the standard error

baselines <- summaries %>%
  filter(str_detect(cond, "BP"), experiment %in% c(1, 2)) %>%
...
baselines_vol <- summaries %>%
  filter(str_detect(cond, "BV"), experiment %in% c(1, 2)) %>%
  mutate(phys1 = 20,
         phys2 = 4) %>%
  group_by(IdLabel) %>%
  mutate(lapse = mean(sampling) * 2)
...
data.frame(model = 1:6,
           description = c("hurdle", "random dimension",
                           ...
                           "volume first", "scalar expected value")) %>%
