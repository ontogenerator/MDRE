...
alpha_1 <- 0.05
# n_compare <- 4
# alpha_2 <- 4 * alpha_1 / (n_compare)^2 # alpha corrected for multiple comparisons
...
sev_fit <- function(p1, p2, gamma, lapse) {
  pnorm(0, mean = p1 - p2, sd = lapse + gamma * sqrt(p1^2 + p2^2), lower.tail = FALSE)
}
...
baselines <- summaries %>%
  ...
  mutate(p1 = 0.5,
         p2 = 0.2,
         v1 = if_else(str_detect(cond, "1"), 4, 20),
         v2 = map_dbl(v1, ~ . + rnorm(1))) %>%
  group_by(IdLabel) %>%
    mutate(relev_perf = 1 - sampling)


gammas <- baselines %>%
  group_by(IdLabel, cohort) %>%
  group_nest() %>%
  mutate(gamma_sev = map_dbl(data, 
                         ~coef(nls(performance ~ 
                                     sev_fit(p1, p2, gamma, lapse = 0),
                                   weights = Nrel,
                                   start = c(gamma = 0.5), 
                                   lower = c(gamma = 0),
                                   upper = c(gamma = 10),
...
median_gamma_vol <- gammas_vol %>%
   group_by(cohort != 2) %>%
   summarise(med_gamma = median(gamma_sev), mad_gamma = mad(gamma_sev))

med_g_vol <- gammas_vol %>%
  ungroup() %>%
  summarise(med_gamma = median(gamma_sev), mad_gamma = mad(gamma_sev))
...
mean_rew <- conds_tab %>%
  ...
  summarise(mean_rew = (ret + ret2)/2)

# summaries %>%
#   filter(experiment %in% 1:3) %>%
#   inner_join(mean_rew) %>% 
#   ggplot() +
#   geom_point(aes(mean_rew, sampling, color = IdLabel)) +
#   facet_grid(experiment ~ .) +
#   stat_smooth(aes(mean_rew, sampling, color = IdLabel), method = lm, se = FALSE)
...
pl <- tbl %>%
  ggplot(aes(gamma, choice, color = cond)) +
  geom_point() +
  geom_smooth() +
...
# Acknowledgments
We thank Miléna Brunet, Alexia Hyde, and Sabine Wintergerst for data acquisition, Katja Frei for assistance with the mice, and Alexej Schatz for programming of the control software.   
