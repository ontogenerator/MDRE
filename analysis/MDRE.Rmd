...
miceChoices <- read.csv2(file = "ChoicesOutput.csv", header = TRUE, dec = ".", sep = ";",
                            na.strings = "NA") %>%
...
miceChoices <- miceChoices %>%
  ...
  mutate(count = 1:n(), revcount = n():1, cohort = factor(cohort)) %>%
...
```{r resoverview, echo = FALSE, fig.cap="(ref:labresoverview)", echo = FALSE}
...
```{r exp1, echo = FALSE, fig.cap="(ref:labresexp1)", echo = FALSE}
exp1_simpl <- exp1 %>%
  ungroup() %>%
...
exp1_diff %>%
  ...
  xlab("") + scale_color_manual(values = cbbPalette)
```


(ref:labresexp2) **Some text about the results from Experiment 2**. More text after that

```{r exp2, echo = FALSE, fig.cap="(ref:labresexp2)", echo = FALSE}
exp2_simpl <- exp2 %>%
  ungroup() %>%
  mutate(cond = as.factor(ifelse(str_length(cond) == 4, str_sub(cond, 1, 2),
                                 as.character(cond)))) %>%
...
(ref:labresexp3) **Some text about the results from Experiment 3**. More text after that

```{r exp3, echo = FALSE, fig.cap="(ref:labresexp3)", echo = FALSE}
...
```{r appfig1, echo = FALSE, fig.cap="(ref:labappfig1)", echo = FALSE}
alpha_14 <- 0.05/5
...
```{r appfig2, echo = FALSE, fig.cap="(ref:labappfig2)", echo = FALSE}
