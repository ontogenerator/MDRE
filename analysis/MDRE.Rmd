...
---
Vladislav Nachev^1\*$\dagger$\S^, Marion Rivalan^1,2$\dagger$&#182;^, York Winter^1,2\S^
...
[@matell_timing_2014]
[@pitz_judgment_1984] Compensatory and non-compensatory decision-making 
[@hunt_hierarchical_2014]
[@reid_information_2015]
...
annotation_1 <- tibble(x = 1, y = c(1, 0), label = c("higher volume preferred \n(higher expected value)",
...
e1p <- exp1_simpl %>%
  ...
  ylim(c(0, 1)) +
  geom_text(data = annotation_1, aes(x = x, y = y, label = label))
...
In order to extend the basic model for multi-dimensional choice situations, we implemented six variations that differed in the integration of information from the volume and probability dimensions (Table \@ref(tab:modTable)). The information from the different reward dimensions was used to obtain for each choice option a *remembered value* (utility), which exhibited the scalar property. Choice was simulated by single sampling from the remembered value distributions with means equal to the remembered values and standard deviations proportional to the remembered values. 
The *remembered value* in the scalar expected value and two-scalar models relied on the full information of both the volume and the probability dimensions, but differed in the implementation of the scalar property, which either affected only the volume dimension (scalar expected value) or both dimensions (two-scalar; Table \@ref(tab:modTable)). In the random dimension model, the *remembered value* for each choice was determined by only one of the reward dimensions, selected with a certain probability, an additional free parameter. In the winner-takes-all model, choice was exclusively driven by the more salient of the two dimensions. In the last two models the saliences of the reward dimensions were considered one after the other, either probability first, or volume first, and a decision was reached if the salience surpassed a given threshold, estimated in previous discrimination experiments [@rivalan_principles_2017].  
...
|model | description | remembered value | criterion | $\gamma$ |
|------|-------------|------------------|-----------|----------|
|sev | scalar expected value | $\pi \mathcal{N}(v,\gamma v)$ | - | 0.95 |
|2scal | two-scalar | $\mathcal{N}(\pi, \gamma \pi)\times \mathcal{N}(v, \gamma v)$ | - | 0.6 |
|rd | random dimension      | $\mathcal{N}(r, \gamma r)$ | $\theta_v$ = 0.25 | 0.5 |
|wta | winner-takes-all      | $\mathcal{N}(r, \gamma r)$ | $\theta$ = 1 | 0.75 |
...
rd_fit <- function(p1, p2, v1, v2, gamma_p, gamma_v, lapse, p_vol) {
  pnorm(0, mean = v1 - v2, sd = lapse + gamma_v * sqrt(v1^2 + v2^2), lower.tail = FALSE) * p_vol + (1 - p_vol) * pnorm(0, mean = p1 - p2, sd = lapse + gamma_p * sqrt(p1^2 + p2^2), lower.tail = FALSE)
}



wta_fit <- function(p1, p2, v1, v2, gamma, lapse) {
  p_sal <- rint(c(p1, p2))
  v_sal <- rint(c(v1, v2))
  
  case_when(
    p_sal > v_sal ~ pnorm(0, mean = p1 - p2, sd = lapse + gamma * sqrt(p1^2 + p2^2), lower.tail = FALSE),
    v_sal > p_sal ~ pnorm(0, mean = v1 - v2, sd = lapse + gamma * sqrt(v1^2 + v2^2), lower.tail = FALSE),
    TRUE ~ 0.5
    )
}
...
baselines <- summaries %>%
  ...
  mutate(p1 = ifelse(experiment == 2, 1, 0.5),
         p2 = 0.2,
  ...
  mutate(gamma_sev = map_dbl(data, 
                         ~coef(nls(performance ~ 
                                   ...
                                   control = list(maxiter = 50000,
                                                  minFactor = 1/2000))))
         # ,
         #     gamma_wta = map_dbl(data,
         #                 ~coef(nls(performance ~
         #                             wta_fit(p1, p2, v1, v2, gamma, lapse = 0),
         #                           weights = Nrel,
         #                           start = c(gamma = 0.7),
         #                           lower = c(gamma = 0),
         #                           upper = c(gamma = 10),
         ...
         #                           control = list(maxiter = 50000,
         #                                          minFactor = 1/2000)))) 
         # gamma_rd_p = map_dbl(data, 
         #                 ~coef(nls(performance ~ 
         #                             rd_fit(p1, p2, v1, v2, gamma_p, gamma_v, lapse = 0, p_vol),
         #                           weights = Nrel,
         #                           start = c(gamma_p = 0.5, gamma_v = 0.5, p_vol = 0.5), 
         #                           lower = c(gamma_p = 0, gamma_v = 0, p_vol = 0),
         #                           upper = c(gamma_p = 10, gamma_v = 10, p_vol = 1),
         #                           data = .x,
         #                           algorithm = "port", 
         #                           control = list(maxiter = 50000,
         #                                          minFactor = 1/2000)))[1]),
         # p_vol = map_dbl(data, 
         #                 ~coef(nls(performance ~ 
         #                             rd_fit(p1, p2, v1, v2, gamma, lapse = 0, p_vol),
         #                           weights = Nrel,
         #                           start = c(gamma = 0.5, p_vol = 0.5), 
         #                           lower = c(gamma = 0, p_vol = 0.3),
         #                           upper = c(gamma = 10, p_vol = 0.9),
         #                           data = .x,
         #                           algorithm = "port", 
         #                           control = list(maxiter = 50000,
         #                                          minFactor = 1/2000)))[2])
...
sims_data <- read.csv2(file = "data/simulations.csv",
  ...
  mutate(model = factor(model, levels = c("sev", "2scal", "rd", "wta", "pfirst", "vfirst")))
...
3. *Random dimension model*. Each decision is based on a single dimension, selected with probability $\theta_v$, a free paremeter.    
...
All models described above share the same free parameter, the scalar factor $\gamma$. In order to obtain baseline estimates for $\gamma$ (Table \@ref(tab:conds_tab)), we focused on the probability baseline discrimination performances of all mice in experiments 1, and 4 (conditions BPV1 and BPV2). Wherever possible we directly fit the models to these data or otherwise performed a sensitivity analysis by varying $\gamma$ with steps of 0.05 in the range of (0.05, 2) and selected the values that resulted in outputs closest to the empirical baseline. Similarly, we performed a sensitivity analysis of the $\theta_v$ free parameter in the random dimension model in the range of (0, 1), with a step of 0.05. The resulting free parameter estimates (across animals) were then used in out-of-sample tests of the six models. For each of the experimental conditions in the four experiments (Table \@ref(tab:conds_tab)) and for each of the six models we simulated 100 choices by 100 (identically parametrized) mice. Over the 100 choices we calculated the discrimination performance for each mouse and then used the median of the individual discrimination performances as the model prediction. We then quantified the model fits to the empirical data by calculating root-mean-square-errors (RMSE), excluding the BPV1 and BPV2 conditions in experiments 1 and 4. Finally, we ranked the models by their RMSE scores.
...
M.R. Methodology, Writing—original draft, Writing—review and editing, Supervision.  
