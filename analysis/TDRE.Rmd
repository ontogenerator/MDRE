...
---
Vladislav Nachev^1\*^, Marion Rivalan^1,2^, York Winter^1,2^

^1^ Institute of Biology, Humboldt University, Philippstr. 13, 10099 Berlin, Germany  
^2^ Exzellenzcluster NeuroCure, Charité University Medicine, Virchowweg 6, 10117 Berlin, Germany
...
First, we picked *a priori* a smallest effect size of interest (sesoi) as the difference in discrimination performance of `r sesoi` units in either direction. This value was chosen based on standard deviations (sd) in discrimination performance observed in previous studies [e.g. Fig. 4 in @rivalan_principles_2017], which ranged from 0.05 to 0.1. Although discrimination performance is bound by 0 and 1, most empirical values, especially the differences between two values, are far enough from these bounds so that their distribution approaches the normal. The expected sd of the difference between two normal distributions with sd of 0.1 (we conservatively picked the largest value) is $\sqrt{0.1^2 + 0.1^2} = 0.141$. With this standard deviation and a sample size of `r n_inds`, the equivalence bounds needed to detect equivalence of paired samples with a power of `r goal_power` are `r glue("[{-sesoi}, {sesoi}]")` [`powerTOSTpaired.raw` function in package `TOSTER`, @lakens_equivalence_2017]. The sesoi can be graphically represented as the `r glue("[{-sesoi}, {sesoi}]")` interval around the difference of zero, or as `r glue("[{0.5 - sesoi}, {0.5 + sesoi}]")` around the chance performance of 0.5.  
...
```{r simRankTable, tab.cap="Best performing models ranked by root-mean-square-errors (RMSE)."}
ranks %>% 
  flextable() %>% 
  add_header_row(values = c("", "experiment"), colwidths = c(2, 3)) %>% 
  theme_booktabs() %>% 
  bold(part = "header")
...
```{r coh13RankTable, tab.cap="Best performing models ranked by root-mean-square-errors (RMSE) for cohorts 1 and 3."}
ranks_coh13 %>%
  flextable() %>% 
  add_header_row(values = c("", "experiment"), colwidths = c(2, 3)) %>% 
  theme_booktabs() %>% 
...
```{r coh2RankTable, tab.cap="Best performing models ranked by root-mean-square-errors (RMSE) for cohort 2."}
ranks_coh2 %>%
  flextable() %>% 
  add_header_row(values = c("", "experiment"), colwidths = c(2, 3)) %>% 
  theme_booktabs() %>% 
